...
function getDayName(date) {
  ...
  return arrayDays[newDate.getUTCDay()];
}
...
function getNextFriday(date) {
  const currentWeekIndex = date.getUTCDay();
  const curDate = new Date(date);

  const nextFriday = new Date(date);

  if (currentWeekIndex === 5) {
    nextFriday.setUTCDate(curDate.getUTCDate() + 7);

    return nextFriday;
  }

  nextFriday.setUTCDate(
    ((5 - currentWeekIndex + 7) % 7) + curDate.getUTCDate()
  );
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const diff = dateSecond - dateFirst;
...
function isDateInPeriod(date, period) {
  ...
  return periodStartDate <= searchDate && searchDate <= periodEndDate;
}
...
function formatDate(date) {
  ...
  if (newDate.getUTCHours() === 12) {
    diffTime = newDate.getUTCHours();
    timePart = 'PM';
  } else if (newDate.getUTCHours() > 12) {
    diffTime = newDate.getUTCHours() - 12;
    timePart = 'PM';
  } else {
    diffTime = newDate.getUTCHours();
    timePart = 'AM';
  ...
  const timeString = `${diffTime.toString()}:${newDate.getUTCMinutes().toString().padStart(2, 0)}:${newDate.getUTCSeconds().toString().padStart(2, 0)} ${timePart}`;

  return `${newDate.getUTCMonth() + 1}/${newDate.getUTCDate()}/${newDate.getUTCFullYear()}, ${timeString}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    const currentUTCDay = currentDay.getDay();
...
function getWeekNumberByDate(date) {
  ...
  let currentDay = basicDate.getUTCDay();
  if (currentDay === 0) {
    currentDay = 7;
  }

  basicDate.setUTCDate(basicDate.getUTCDate() - currentDay + 1);

  const startYear = new Date(Date.UTC(basicDate.getUTCFullYear(), 0, 4));
  let startDay = startYear.getUTCDay();
  if (startDay === 0) {
    startDay = 7;
  }
  const firstWeekStart = new Date(startYear);
  firstWeekStart.setUTCDate(1, 1);

  const diff = basicDate - firstWeekStart;

  const divider = 7 * 24 * 60 * 60 * 1000;

  const weekNumber = Math.floor(diff / divider) + 1;
